...
ConvertWidget::ConvertWidget(QWidget* parent) :QWidget(parent) {
	...
	openSave = new QPushButton("打开文件夹");
	selectSave = new QPushButton("设置文件夹");
	labSavePath = new QLabel(Settings::getInstance()->getSavePath());
	allStart = new QPushButton("全部开始");
	allStop = new QPushButton("全部停止");
	...
	connect(selectSave, &QPushButton::clicked, [this]() {
		QString imageFolder = QFileDialog::getExistingDirectory(this, "选择文件保存路径", "./", QFileDialog::ShowDirsOnly);
...
void ConvertWidget::importFile() {
	...
	if (type == "aax" || type == "aa") {
		...
		QMessageBox msgBox("错误",		///--这里是设置消息框标题
			"不是有效的Audible文件",						///--这里是设置消息框显示的内容
